...
def run_metis_tse(
    ...
    """Run Amphion/Metis TSE through an external command template."""
    template = os.environ.get("METIS_TSE_CMD")
    if not template:
        raise ModelUnavailableError("METIS_TSE_CMD is not configured")
    cmd = format_command(
        template,
